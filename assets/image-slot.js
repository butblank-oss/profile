...
(() => {
  ...
  // The host caps each state file at ~2MB, so a single sidecar tops out at a
  // few dozen screenshots. Shard across SHARDS files instead — the writes are
  // still allowlisted (*.state.json) and only the touched shard is rewritten,
  // which also makes each save far cheaper than re-serializing everything.
  const SHARDS = 24;
  const shardFile = (i) => '.image-slots.' + i + '.state.json';
  function shardOf(id) {
    let h = 5381;
  ...
  // Which shard each hydrated id came from, so a later clear can rewrite the
  // right file (legacy ids live in the unsharded sidecar until next write).
  const homeShard = {};

  function load() {
    if (loadP) return loadP;
    const files = [fetch(STATE_FILE).then((r) => (r.ok ? r.json() : null)).catch(() => null)];
    for (let i = 0; i < SHARDS; i++) {
      files.push(fetch(shardFile(i)).then((r) => (r.ok ? r.json() : null)).catch(() => null));
    }
    loadP = Promise.all(files)
      .then((all) => {
        const j = {};
  ...
  // Group the current slots by target shard. An id keeps whichever shard it
  // was hydrated from so a value never ends up written to two files.
  function buildShards() {
    const out = {};
    for (const id in slots) {
      const i = homeShard[id] === undefined ? shardOf(id) : homeShard[id];
      (out[i] = out[i] || {})[id] = slots[id];
    }
    return out;
  }

  // Shards that need rewriting. A cleared id's old shard must be rewritten
  // even though it holds no current slots, or the stale value survives.
  const dirtyShards = new Set();

  function writeShards(w, indices) {
    const groups = buildShards();
    return Promise.all(indices.map((i) => {
      const body = JSON.stringify(groups[i] || {});
      return Promise.resolve(w(shardFile(i), body)).catch((err) => {
        console.error('<image-slot> could not persist ' + shardFile(i) +
          ' (' + Math.round(body.length / 1024) + ' KB). Dropped images will not survive a reload.', err);
      });
    }));
  }
  ...
  function flushNow() {
    ...
    if (!w) return;
    const idx = dirtyShards.size ? Array.from(dirtyShards) : Object.keys(buildShards()).map(Number);
    dirtyShards.clear();
    try { writeShards(w, idx); } catch (e) {}
  }
  function save() {
  ...
  function setSlot(id, val) {
    if (!id) return;
    dirtyShards.add(homeShard[id] === undefined ? shardOf(id) : homeShard[id]);
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      // Encode at the full cap regardless of the slot's rendered width: these
      // slots are small thumbnails but the lightbox shows the image near
      // full-screen, so slot-width-derived caps looked blurry when zoomed.
      const cap = MAX_DIM;
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '<div class="ctl" popover="manual" data-dc-edit-transparent><button data-act="replace" title="Replace image">교잴</button>' +
        '  <button data-act="edit" title="Reframe image">편집</button>' +
        '  <button data-act="clear" title="Remove image">삭제</button></div>' +
      ...
      root.addEventListener('click', (e) => {
        ...
        if (act === 'replace') {
          ...
          // Local file input: the host-owned pick event is not handled in every
          // embedding, and Replace has to work regardless.
          this._input.click();
        }
        if (act === 'clear') {
          this._exitReframe(true);
